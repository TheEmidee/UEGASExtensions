...
void UGASExtAbilitySystemComponent::OnRep_ReplicatedAnimMontageForMesh()
{
    for ( auto & new_rep_montage_info_for_mesh : RepAnimMontageInfoForMeshes )
    {
        ...
        if ( !AbilityActorInfo->IsLocallyControlled() )
        {
            ...
            if ( new_rep_montage_info_for_mesh.RepMontageInfo.GetAnimMontage() )
            {
                // New Montage to play
                if ( ( anim_montage_info.LocalMontageInfo.AnimMontage != new_rep_montage_info_for_mesh.RepMontageInfo.GetAnimMontage() ) ||
